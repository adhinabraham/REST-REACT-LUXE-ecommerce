...
function Cart() {
  ...
  const appycoupon = () => {
    ...
    const data = { "coupon_code": couponcode, "status": "appled", "user_name": userid }
    axios.post("http://127.0.0.1:8000/cart/couponcheck/", data).then((Response) => {
      console.log("this is coupon then")
      console.log(Response.data)
      setpercentage(Response.data["saved"])
